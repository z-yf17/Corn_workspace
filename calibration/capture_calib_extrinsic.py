...
# 预览检测策略（降低 CPU）：每 N 帧检测一次 + 检测时缩小灰度图
DETECT_EVERY_N = 2           # 2=15Hz, 3=10Hz（建议 2 或 3）
PREVIEW_DETECT_SCALE = 0.5   # 0.5 或 0.33（越小越省 CPU，但只影响预览，不影响保存精度）

# 预览窗口显示缩放（降低 imshow 开销）
SHOW_SCALE = 0.5
...
OUTDIR = "./handeye_data_rs_1080p_sync"
...
def setup_realsense_force_1080p_30():
    ctx = rs.context()
    ...
    print(f"[RS] device: {name} S/N: {serial}")
    ...
    return pipeline, (intr.width, intr.height), K, dist, need_rgb2bgr
...
def setup_zmq_sub():
    ...
    sub.setsockopt(zmq.RCVHWM, 200)  # 别无限堆积
...
def drain_joint_messages(sub, joint_buf: deque, max_msgs: int = 500):
    """
    单次最多处理 max_msgs 条，避免赶队列占满 CPU。
    兼容：
      - 推荐：t_meas_ns（monotonic ns）
      - 旧版：t（time.time 秒），仅做兼容
    """
    n = 0
    while n < max_msgs:
        try:
...
def grab_latest_color_frame(pipeline, need_rgb2bgr: bool):
    """
    取尽量“最新”的 frame：wait 1帧 + poll 丢旧帧，只保留最后一帧
    """
...
def detect_markers(frame_bgr, detect_fn, scale: float):
    """
    在灰度图上检测：
      - scale<1：灰度缩小后检测，再把 corners 缩放回原图坐标
      - scale=1：原图灰度直接检测（最准确）
    返回 corners(原图坐标), ids, rejected
    """
...
def main():
    ...
    pipeline, (W, H), K, dist, need_rgb2bgr = setup_realsense_force_1080p_30()
    ...
    cv2.namedWindow("collect (s=save q=quit)", cv2.WINDOW_NORMAL)
    ...
    try:
        while True:
            ...
            if time.time() - t_hb > 1.0:
                ...
                print(f"[HB] idx={idx} joint_buf={jb} last_seq={last_seq}")
                t_hb = time.time()
            ...
            if key in (ord(KEY_SAVE), ord(KEY_SAVE.upper())):
                ...
                print(f"[SAVE] {npz_path} (mode={m['mode']} err={err_ms:.1f}ms)")
                idx += 1
