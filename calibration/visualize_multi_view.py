...
# ===================== 配置（稳定优先：保守参数） =====================
CAMERA_ROLES  = ["right", "left"]     # ✅ 多相机：同时显示这两路
...
EXTR_NPZ_TMPL = "./calib_extrinsic/{role}/fr3_realsense_eye_to_hand.npz"
...
# 点云：stride + 限点数（每路相机都应用）
PC_STRIDE   = 4
DEPTH_MIN_M = 0.20
DEPTH_MAX_M = 2.00
MAX_POINTS_PER_CAM = 12000

# Open3D：整体降频（降低桌面卡顿风险）
...
def rs_worker(
    ...
    # 子进程内 import，避免主进程/子进程 OpenGL 相关干扰
        ...
        try:
            ...
            for dev in ctx.query_devices():
                try:
                    ...
                    continue
                if s == serial_:
                    print(f"[RS][{role}][RESET] hardware_reset serial={serial_}", flush=True)
                    dev.hardware_reset()
                    return True
        except Exception as e:
            print(f"[RS][{role}][RESET] failed: {e}", flush=True)
        return False
    ...
    def start_pipe() -> bool:
        ...
        try:
            ...
            dev = prof.get_device()
            try:
                print(f"[RS][{role}] using {dev.get_info(rs.camera_info.name)} S/N:{dev.get_info(rs.camera_info.serial_number)}", flush=True)
            except Exception:
                pass
            ...
            print(f"[RS][{role}] started {w}x{h}@{fps} depth_scale={depth_scale}", flush=True)
            return True
        except Exception as e:
            print(f"[RS][{role}] start failed: {e}", flush=True)
            stop_pipe()
            return False
    ...
    while not stop_evt.is_set():
        ...
        try:
            ...
            print(f"[RS][{role}] runtime error:\n{traceback.format_exc()}", flush=True)
            stop_pipe()
            time.sleep(0.2)

    stop_pipe()
    print(f"[RS][{role}] exit", flush=True)


def start_cam_process(role: str, serial: str, T_base_from_cam: np.ndarray) -> Tuple[mp.Process, mp.Queue, mp.Event]:
    q = mp.Queue(maxsize=1)
    evt = mp.Event()
    p = mp.Process(
        target=rs_worker,
        args=(q, evt, role, serial, T_base_from_cam,
              RS_W, RS_H, RS_FPS, PC_STRIDE, DEPTH_MIN_M, DEPTH_MAX_M, MAX_POINTS_PER_CAM),
        daemon=True
    )
    p.start()
    return p, q, evt
...
def main():
    ...
    # per-cam: serial + extrinsic(base<-cam)
    cam_serial: Dict[str, str] = {}
    cam_T_base_from_cam: Dict[str, np.ndarray] = {}
    for role in CAMERA_ROLES:
        serial = load_serial_from_mapping(role, MAPPING_JSON)
        if not serial:
            raise RuntimeError(f"Mapping invalid: {MAPPING_JSON}, need key '{role}'")
        cam_serial[role] = serial

        extr_npz = EXTR_NPZ_TMPL.format(role=role)
        if not os.path.exists(extr_npz):
            raise RuntimeError(f"Extrinsic npz not found for {role}: {extr_npz}")
        ...
        cam_T_base_from_cam[role] = inv_T(T_cam_from_base)

        print(f"[CFG] role={role} serial={serial} extr={extr_npz}")
    ...
    for lname, info in links.items():
        ...
        link_mesh[lname] = dict(mesh=mesh, V0=V0, T_link_mesh=info["T_link_mesh"])
        mesh_count += 1
    ...
    vis.create_window(window_name=f"Mesh + Multi-Cloud {CAMERA_ROLES}", width=1280, height=720)
    try:
    ...
    # 每路相机一个 pcd
    pcd_map: Dict[str, o3d.geometry.PointCloud] = {}
    for role in CAMERA_ROLES:
    ...
    # 启动相机进程
    cam_proc: Dict[str, mp.Process] = {}
    cam_q: Dict[str, mp.Queue] = {}
    cam_evt: Dict[str, mp.Event] = {}
    last_cam_ts: Dict[str, float] = {role: 0.0 for role in CAMERA_ROLES}
    did_reset_view = False

    def start_one(role: str):
        p, q, evt = start_cam_process(role, cam_serial[role], cam_T_base_from_cam[role])
        cam_proc[role] = p
        cam_q[role] = q
        cam_evt[role] = evt
        last_cam_ts[role] = 0.0

    def stop_one(role: str):
        if role in cam_proc:
            stop_cam_process(cam_proc[role], cam_evt[role], timeout_sec=2.0)
            cam_proc.pop(role, None)
            cam_q.pop(role, None)
            cam_evt.pop(role, None)
    ...
    def restart_one(role: str):
        nonlocal did_reset_view
        print(f"[WATCHDOG] restarting camera role={role} ...")
        stop_one(role)
        start_one(role)
        did_reset_view = False
    ...
    def restart_all():
        nonlocal did_reset_view
        print("[WATCHDOG] restarting ALL cameras ...")
        for role in list(CAMERA_ROLES):
            stop_one(role)
        for role in list(CAMERA_ROLES):
            start_one(role)
        did_reset_view = False
    ...
    # key: q/ESC exit, r restart all cams
    ...
    try:
        while not STOP:
            ...
            for role in CAMERA_ROLES:
                pkt = None
                try:
                    pkt = cam_q[role].get_nowait()
                except Exception:
                    pkt = None

                if pkt is not None:
                    last_cam_ts[role] = pkt.ts
                    if pkt.pts_base.shape[0] > 0:
                        p = pcd_map[role]
                        p.points = o3d.utility.Vector3dVector(pkt.pts_base.astype(np.float64))
                        p.colors = o3d.utility.Vector3dVector(pkt.cols_rgb.astype(np.float64))
            ...
            tnow = time.time()
            for role in CAMERA_ROLES:
                ts = last_cam_ts.get(role, 0.0)
                if ts > 0 and (tnow - ts) > CAM_STALE_SEC:
                    restart_one(role)
