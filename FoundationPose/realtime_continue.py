...
# ---- periodic re-register to avoid drift ----
REESTIMATE_ENABLE = True
REESTIMATE_INTERVAL_SEC = 1.0         # 每 1 秒 register 一次
REESTIMATE_REFINE_ITER = 3            # 周期性 register 可以用更小迭代，减少抖动/耗时（你也可设回 EST_REFINE_ITER）
...
def bind_anchor_auto(T, p_cam, n_cam):
    """
    自动判断 T 是 obj->cam 还是 cam->obj，并返回绑定到物体系 (p_obj, n_obj)。
    返回: (dir_str, p_obj, n_obj, errA, errB)
    """
    ...
    n_obj_A = R.T @ n_cam
    n_obj_A /= (np.linalg.norm(n_obj_A) + 1e-12)
    ...
    n_obj_B = R @ n_cam
    n_obj_B /= (np.linalg.norm(n_obj_B) + 1e-12)
...
def make_cylinder_between(p, q, radius=0.004, resolution=24):
    ...
    if axis_norm < 1e-9:
        if float(np.dot(z_axis, v_unit)) < 0.0:
            Rm = o3d.geometry.get_rotation_matrix_from_axis_angle(np.array([np.pi, 0.0, 0.0], dtype=np.float64))
        else:
            Rm = np.eye(3)
    else:
        axis = axis / axis_norm
...
def main():
    ...
    tracking = False
    need_register = False
    last_register_time = 0.0   # 上次 register 成功/尝试的时间
    last_register_note = ""
    ...
    try:
        while True:
            ...
            # ---------- Pose (track every frame; register on-demand + periodic) ----------
            ...
            if rgb_bgr is not None:
                ...
                if need_register and mask_pose_bool.sum() > MIN_MASK_PIX_FOR_REGISTER:
                    pose_try = est.register(K, rgb_rgb, depth_m, mask_pose_bool, EST_REFINE_ITER)
                    last_register_time = now_t
                    last_register_note = "REGISTER(S)"
                ...
                if tracking and pose is not None:
                    pose_try = est.track_one(rgb_rgb, depth_m, K, TRACK_REFINE_ITER)
                    if pose_try is not None:
                        pose = pose_try
                    else:
                        # 追踪失败：保留 pose（也可以设 pose=None 触发冻结/重注册）
                        last_register_note = "TRACK_FAIL"

                # 3) 周期性 re-register（防漂移）
                if (REESTIMATE_ENABLE and tracking and (pose is not None)
                        and (now_t - last_register_time) >= float(REESTIMATE_INTERVAL_SEC)
                        and (mask_pose_bool.sum() > MIN_MASK_PIX_FOR_REGISTER)):
                    pose_try = est.register(K, rgb_rgb, depth_m, mask_pose_bool, REESTIMATE_REFINE_ITER)
                    last_register_time = now_t
                    last_register_note = f"REREG({REESTIMATE_INTERVAL_SEC:.1f}s)"
                    if pose_try is not None:
                        pose = pose_try
            ...
            pose_str = f"pose={'Y' if pose is not None else 'N'} track={int(tracking)} {last_register_note}"

            put_text(
                vis_bgr,
                f"M{int(ctrl['mask_refine'])} B{int(ctrl['depth_band'])} C{int(ctrl['cluster_filter'])} {band_str} | "
                f"reg[S] | {pick_str} | reReg={int(REESTIMATE_ENABLE)}@{REESTIMATE_INTERVAL_SEC:.1f}s | V/H/T/P | Q/ESC",
                30
            )
            put_text(vis_bgr, pose_str, 60, color=(255, 255, 255))
            ...
            if key in (ord('s'), ord('S')):
                ...
                tracking = False
                pose = None
                last_register_note = "WAIT_REGISTER(S)"
